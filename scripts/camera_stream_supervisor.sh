...
write_state() {
  cat > "${STATE_FILE}" <<EOF
STREAM_ENABLE_LOCAL=${ENABLE_LOCAL:-0}
STREAM_LOCAL_PORT=${LOCAL_PORT:-5600}
STREAM_LOCAL_HOST=${LOCAL_HOST:-127.0.0.1}
STREAM_REMOTE_HOST=${REMOTE_HOST:-}
STREAM_REMOTE_PORT=${REMOTE_PORT:-5601}
STREAM_WIDTH=${WIDTH:-1296}
STREAM_HEIGHT=${HEIGHT:-972}
STREAM_FPS=${FPS:-15}
STREAM_CAM_PORT=${CAM_PORT:-${LOCAL_PORT:-5600}}
STREAM_CAMERA_REMOTE_PORT=${CAMERA_REMOTE_PORT:-${REMOTE_PORT:-5601}}
EOF
}
